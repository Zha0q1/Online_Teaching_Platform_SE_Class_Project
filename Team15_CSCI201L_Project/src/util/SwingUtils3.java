...
/**
 * source code: http://www.aurelienribon.com/blog/2012/07/tutorial-beautifying-the-dialogs-in-swing-applications/
 * with modifications
 */
public class SwingUtils3 {
	/**
	...
	public static void createDialogBackPanel(JDialog dialog, Component parent) {
		...
		addEscapeToCloseSupport(dialog, true);
	}
	...
	public static void fadeIn(final JDialog dialog) {
		...
		timer.addActionListener(new ActionListener() {
			...
			@Override public void actionPerformed(ActionEvent e) {
				opacity += 0.085f;//0.15f;
				dialog.setOpacity((float) Math.min(opacity, .8));//1));
	...
	private static class DialogBackPanel extends JPanel {
		...
		private static final ImageIcon shadowImage = new ImageIcon(SwingUtils3.class.getResource(Constants.shadow));
		...
		public DialogBackPanel(JDialog dialog) {
			...
			title.setFont(new Font("SquareFont", Font.PLAIN, 15));
			title.setForeground(Color.WHITE);
			info.setFont(new Font("SquareFont", Font.PLAIN, 10));
